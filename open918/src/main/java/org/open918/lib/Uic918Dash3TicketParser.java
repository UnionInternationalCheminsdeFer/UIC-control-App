...
public class Uic918Dash3TicketParser {
    ...
    public static Ticket918Dash3 parse(byte[] data) throws DataFormatException, ParseException {
        String contents = new String(data, UicTicketParser.UTF8);
        ...
        if (ticket.getContents() == null || ticket.getContents().getFields().size() == 0) {
            decodeBody(data, new String(data, UicTicketParser.ASCII), ticket);
        }
    ...
    static int parseHeader(Ticket918Dash3 s, String body, int offset) throws ParseException{
        s.getHeader().setVersion(Strings.getInteger(body, offset, 0,6));
        s.getHeader().setLength(Strings.getInteger(body, offset, 2,6));
        s.getHeader().setCarrier(Strings.getInteger(body, offset, 6, 10));
        s.getHeader().setOrderNumber(Strings.getString(body, offset,10, 30));

        s.getHeader().setCreationDate(sdf.parse(Strings.getString(body, offset, 30,42)).getTime());
        s.getHeader().setFlags(TicketFlag.getFlagsFromNumber(Strings.getInteger(body, offset, 42, 43)));

        s.getHeader().setLanguage(Strings.getString(body, offset, 43,45));
        s.getHeader().setLanguage2(Strings.getString(body, offset,  45, 47));
        return offset + 47;
    }
    ...
    static int parseFields(Ticket918Dash3 s, String body, int initialOffset, String type) throws ParseException {
        ...
        try {
            ...
            for (int i = 0; i < contents.getNumberOfFields(); i++) {
                ...
                Integer len = Strings.getInteger(body, offset, 9, 13);
                if (len > body.length()) {
