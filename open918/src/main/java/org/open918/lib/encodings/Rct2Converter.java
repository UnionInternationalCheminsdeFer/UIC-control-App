...
public class Rct2Converter extends TicketConverter {
    ...
    /*
        Split multi-line fields into ticket fields of one row each (CD tickets have these)
     */
    private static List<TicketField> cloneAndConvert(List<TicketField> fields) {
        List<TicketField> tf = new ArrayList<>();
        for (TicketField f : fields) {
            if (f.getHeight() > 1) {
                int line = 0;
                String[] splitText = f.getText().split("\n");
                while (line != f.getHeight()) {
                    TicketField field = new TicketField();
                    field.setColumn(f.getColumn());
                    field.setLine(f.getLine()+line);
                    field.setWidth(f.getWidth());
                    field.setHeight(1);
                    field.setFormatting(f.getFormatting());

                    if (line < splitText.length) {
                        field.setText(splitText[line]);
                    } else {
                        field.setText("");
                    }
                    // The length in this case is of the whole field, but we need the length of each line here
                    field.setLength(field.getText().length());
    ...
    private void addField(StringBuffer buffer, int line, int column, String text, int length) {
        ...
        if (offset+length <= buffer.length()) {
            ...
            //throw new IllegalArgumentException("Trying to add text '"+text+"', but failed, as "+(offset+length)+" is greater than "+buffer.length());
        }
